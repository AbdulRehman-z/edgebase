...
export const SpinnerLoadingIndicator = ({
  ...
  return (
    <div className={cn("relative", className)}>
      <StatusBorder className="border-blue-700 border-dashed">{children}</StatusBorder>
      <div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center">
        <LoaderCircle className={cn("size-5 animate-spin text-blue-700 opacity-70")} />
      </div>
    </div>
  );
};
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-px -left-px h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-md border",
          className,
        )}
      />
      {children}
